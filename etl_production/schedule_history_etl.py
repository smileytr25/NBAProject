...
def get_year_game_schedule(year, page_limit, pages_visited=0, start_time=None):
    months = [
        "ocotober", "november", "december", "january", "february", "march", 
        "april", "may", "june"
    ]
...
def check_for_schedules_to_scrape():
    db_path = Path("~/Personal Project/data/nba.db").expanduser()
    engine = create_engine(f"sqlite:///{db_path}")

    with engine.connect() as conn:
        query = text("SELECT MAX(date) FROM game_schedule_history")
        last_day_in_db = conn.execute(query).fetchone()[0]

        start_of_new_days = next_day_to_check = last_day_in_db + pd.Timedelta(days=1)
...
if __name__ == "__main__":
    game_schedule_history_etl(list(range(1947, 2027)), 19)